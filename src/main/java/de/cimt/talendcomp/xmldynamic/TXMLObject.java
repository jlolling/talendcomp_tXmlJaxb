...
public abstract class TXMLObject implements Serializable, Cloneable{
    ...
    @XmlTransient
    private static final GenCache<Class<?>, Map<String, ExtPropertyAccessor>> CACHE
            = new GenCache<Class<?>, Map<String, ExtPropertyAccessor>>(new GenCache.LookupProvider<Class<?>, Map<String, ExtPropertyAccessor>>() {
                @Override
                public Map<String, ExtPropertyAccessor> lookup(Class<?> k) {

                    return CollectionUtil.generateLookupMap( ReflectUtil.introspect(k, Object.class) , new Transformer<ExtPropertyAccessor, String>() {
                        @Override
    ...
    public void afterUnmarshal(Unmarshaller um, Object parent){
        ...
        catch(Throwable t){} // can only be classcast exception and then there is nothing to do
    }
...
/**
    ...
    protected Object clone() throws CloneNotSupportedException {
        ...
        for(ExtPropertyAccessor pa : CACHE.get( getClass() ).values()){
            pa.setPropertyValue(clone, pa.getPropertyValue(this));
        }
